// SAMPLE
this.i18n = {
	"extension": {
		"en": "JiffyBOX",
		"de": "JiffyBOX"
	},
    "settings": {
        "en": "Settings",
        "de": "Optionen"
    },
    "search": {
        "en": "Search",
        "de": "Suche"
    },
    "nothing-found": {
        "en": "No matches were found.",
        "de": "Keine Übereinstimmungen gefunden."
    },
    
    
    
    "information": {
        "en": "Information",
        "de": "Information"
    },
    "authentication": {
        "en": "Authentication",
        "de": "Authentifizierung"
    },
    "token": {
        "en": "API-Token:",
        "de": "API-Token:"
    },
    "x-characters": {
        "en": "32 characters",
        "de": "32 Zeichen"
    },
    "description": {
        "en": "For script actions of your JiffyBox-accounts an access option is available via an interface. <br> \
		You need the so-called \"API-Token\", which you can find in the Control Panel under <br> \
		<b>\"Stammdaten\" >> \"API-Zugriff\"</b>. <br><br> \
		In order to do so login into your <a href=\"https://admin.jiffybox.de/\" target=\"_blank\" alt=\"JiffyBOX\"> JiffyBOX control panal</a>.",
        
        "de": "Für skriptgesteuerte Bedienung Ihres JiffyBox-Accounts steht eine Zugriffsmöglichkeit über eine Schnittstelle zur Verfügung. <br> \
 		Hierfür	benötigen Sie den sogenannten \"API-Token\", den Sie im Control-Panel unter \"Stammdaten\", \"API-Zugriff\" erstellen können. <br><br> \
		Loggen Sie sich dazu in ihrer <a href=\"https://admin.jiffybox.de/\" target=\"_blank\" alt=\"JiffyBOX\">JiffyBOX Admin Verwaltung</a> an."
    },
    "logout": {
        "en": "Logout",
        "de": "Abmeldung"
    },
    "enable": {
        "en": "Enable",
        "de": "Aktivieren"
    },
    "disconnect": {
        "en": "Disconnect:",
        "de": "Trennen:"
    },
	"details": {
		"en": "Details",
		"de": "Details"
	},
	"verification": {
		"en": "Verification",
		"de": "Verifizierung"
	},
	"verify": {
		"en": "Please insert \"API-Token\"",
		"de": "Bitte geben Sie den \"API-Token\" ein"
	},
	"notifications": {
		"en": "Notifications",
		"de": "Nachrichten"
	},
	"pricing": {
		"en": "Plans & Pricing",
		"de": "Pläne & Preise"
	},
	"behaviour": {
		"en": "Behaviour",
		"de": "Verhalten"
	},
	"starting": {
		"en": "Starting JiffyBOX open TAP with user-defined url",
		"de": "Beim Start einer JiffyBOX &ouml;ffne ein TAP mit Benutzer-Defenierter URL"
	},
	"notification": {
		"en": "Show notifications",
		"de": "Zeige Benachrichtigunen"
	},
	"shutdown": {
		"en": "Closing Chrome shutdown all JiffyBOXes",
		"de": "Beim Schliessen von Chrome alle JiffyBOXes herunterfahren"
	}
};
